...
public class KMeans_GoodStart extends Configured implements Tool {

	private static final Logger logger = LogManager.getLogger(KMeans_GoodStart.class);
	...
	public static void main(final String[] args) {
		...
		try {
			ToolRunner.run(new KMeans_GoodStart(), args);
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(KMeans_GoodStart.class);
		...
		jobCent.setJarByClass(KMeans_GoodStart.class);
		...
		while (!jobDone && iteration <= 10) {
			...
			job1.setJarByClass(KMeans_GoodStart.class);
			...
			new_SSE = job1.getCounters().findCounter(SSE_New.new_SSE).getValue();
	...
	public static class InitCentroidReducer extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(final Text key, final Iterable<Text> values, final Context context)
			...
			Double centroid = 0.0;
			// Good one
			Double range_diff = (double) (max_follower - min_follower) / k;
			Double init_centroid = min_follower + (range_diff / 2);
			context.write(new Text(round(init_centroid, 3).toString()), new Text("0"));

			for (int j = 1; j < k; j++) {
				centroid = init_centroid + (range_diff * j);
				context.write(new Text(round(centroid, 3).toString()), new Text("0"));
			}
		}
	}
	...
	public static class CentroidReducer extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(final Text key, final Iterable<Text> values, final Context context)
			...
			SSE = round(SSE, 3);
			context.getCounter(SSE_New.new_SSE)
					.setValue(context.getCounter(SSE_New.new_SSE).getValue() + SSE.longValue());
